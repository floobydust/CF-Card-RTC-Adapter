;**************************************************************************************************
;*  C02BIOS 3.03 (c)2013-2021 by Kevin E. Maier   *     New Hardware design with the following:   *
...
;*          13/02/2021 (Day/Month/Year)           *         ROM - $FEA0 - $FFFF                   *
;**************************************************************************************************
        PL      66      ;Page Length
...
;                               START OF BIOS CODE                                                *
;**************************************************************************************************
;C02BIOS version used here is 3.03 (updated release)
...
; $FF06 IDE_IDENTIFY    ;Load IDE Identity Data at $0400
...
; To enable a level of benchmarking, two new routines have been added to C02BIOS version 2.02
...
LBA_RD_OK
                BBS2    MATCH,LBA_RD_OK ;Wait for Read completed via ISR (5)
...
CLEAR_BMI       TRB     RTC_CONTROL_B   ;Clear Burst Mode for NVRAM
                RTS                     ;Return to caller
;
...
;
                LDX     #$00            ;Set index to zero, 8 registers to be read
RTC_LOAD_LOOP
                LDA     RTC_SECONDS,X   ;Get Data from RTC
                STA     BUFF_PG0,X      ;Store into RAM
                INX                     ;Increment register count
                CPX     #$08            ;Check for all 8 moved
                BNE     RTC_LOAD_LOOP   :Loop back until all registers are read
;
                LDA     #%10000000      ;Get TE Bit mask
...
;
                ...
                STA     TICKS           ;Reset to start after RTC init (3)
                RTS                     ;Return to caller (6)
;
ADD_16          CLC                     ;Clear carry flag (2)
                ...
                ADC     #$00            ;Add carry to Temp high byte (3)
...
;
;The 16-bit IOCS16 line is routed to the Set Overflow (SO) pin on the CPU, so it can be used in
; the data transfer routine as a handshake signal. NOTE: So far, using the IOCS16 line to the SO
; pin of the W65C02 has not worked properly. Testing has shown that the IOSC16 line does go active
; during a block transfer, but you still need to read the Status Register and test the DRQ bit.
; As a result, there's really no advantage in using the IOCS16 line. Perhaps a future release will
; make use of it, time will tell. Update: I've opted to remove the jumper and connection from the
; IOCS16 line to the SO line to the CPU in the latest hardware adapter version. It's pretty clear
; that there's no actual value in trying to use it, as the Status register must be read for each
; 16-bit word of data, so any access of the IOCS16 line would just be overhead.
;
...
;
LBA_XFER        LDA     IDE_ALT_STATUS  ;Get Status (resets IRQ) (4)
                AND     #%00001000      ;Check for DRQ (2)
...
;
IDE_ALL_DONE    LDA     IDE_ALT_STATUS  ;Get IDE status (resets IRQ) (4)
...
; Note: No time is preloaded, time is 00:00:00 after a cold start.
;
...
BIOS_MSG        .DB     $0D,$0A         ;CR/LF
                .DB     "C02BIOS 3.03"  ;Updated Release Version
                .DB     $0D,$0A         ;CR/LF
                .DB     "13/02/2021"    ;DD/MM/YYYY
                .DB     $0D,$0A         ;CR/LF
